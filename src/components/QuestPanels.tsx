...
export function PixelGemIcon({ className = 'h-[8px] w-[6px]' }: { className?: string }) {
  return (
    <svg viewBox="0 0 6 8" className={`inline-block shrink-0 ${className}`} shapeRendering="crispEdges" aria-hidden="true">
      ...
      {/* Cyan jewel body */}
      <rect x="1" y="1" width="4" height="6" fill="var(--ui-accent)" />
      <rect x="0" y="2" width="6" height="4" fill="var(--ui-accent)" />
      {/* Radiant highlight */}
      <rect x="1" y="1" width="2" height="2" fill="var(--ui-accent-hi)" />
      {/* Sparkle glint */}
...
export function PixelFlameIcon({ className = 'h-[8px] w-[7px]' }: { className?: string }) {
  return (
    <svg viewBox="0 0 7 8" className={`inline-block shrink-0 fill-[var(--ui-danger)] ${className}`} shapeRendering="crispEdges" aria-hidden="true">
      ...
      <rect x="2" y="4" width="3" height="2" fill="var(--ui-gold)" />
    </svg>
  );
}
  ...
  return (
    <div
      ...
      style={{
        ...
        backgroundColor: progress.done ? `${color}22` : 'var(--ui-panel3)',
      }}
    >
